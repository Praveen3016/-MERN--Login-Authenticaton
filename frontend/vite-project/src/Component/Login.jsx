...
const  navigate = useNavigate()
    ...
    const submitHandle = async (e) =>{
        try {
            e.preventDefault();
            // console.log(user);
          const data = await  axios.post("http://localhost:3000/api/user/Login" , user ,{withCredentials: true});
          // console.log( "token bb" +data.data.token)
          // navigate('/')
          setIsAuthenticate(true)
          alert("loged in")
          navigate("/")
        } catch (error) {
            console.log("error in post axios" , error.response.data.message)
